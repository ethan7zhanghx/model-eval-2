...
evalRouter.post('/job', (req: Request, res: Response): void => {
  ...
  logger.info('[eval/job] received eval request', {
    hasEnv: Boolean((testSuite as EvaluateTestSuite).env),
    envKeys: Object.keys(((testSuite as EvaluateTestSuite).env || {}) as Record<string, unknown>),
    providerIds: Array.isArray(testSuite.providers)
      ? (testSuite.providers as Array<{ id?: unknown }>).map((provider) =>
