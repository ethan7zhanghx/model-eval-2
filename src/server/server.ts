...
function sendV1Disabled(res: Response, feature: string): void {
  res.status(404).json({
    error: 'Not enabled in V1 minimal mode',
    feature,
    hint: 'Set PROMPTFOO_V1_MINIMAL_MODE=false to enable the full feature set.',
  });
}
...
export function handleServerError(error: NodeJS.ErrnoException, port: number): void {
  if (error.code === 'EADDRINUSE') {
    logger.error(`Port ${port} is already in use. Do you have another ERNIE Eval instance running?`);
  } else {
...
export function findStaticDir(): string {
  ...
  const cwdDistPath = path.resolve(process.cwd(), 'dist', 'src', 'app');

  // Prefer the built frontend when available.
  // When we run the source server directly (tsx), serving src/app/ will expose raw
  // TypeScript/Vite entrypoints like /src/main.tsx, which the browser cannot execute.
  // For local demo / desktop usage we should serve dist/src/app whenever it exists.
  if (fs.existsSync(path.join(cwdDistPath, 'index.html'))) {
    logger.debug(`Static directory resolved to built app in cwd: ${cwdDistPath}`);
    return cwdDistPath;
  }
...
export function createApp() {
  const app = express();
  const minimalMode = getEnvBool('PROMPTFOO_V1_MINIMAL_MODE', true);
  ...
  app.get('/api/remote-health', async (_req: Request, res: Response): Promise<void> => {
    if (minimalMode) {
      sendV1Disabled(res, 'remote-health');
      return;
    }
  ...
  if (minimalMode) {
    app.use('/api/redteam', (_req: Request, res: Response) => sendV1Disabled(res, 'redteam'));
    app.use('/api/model-audit', (_req: Request, res: Response) =>
      sendV1Disabled(res, 'model-audit'),
    );
    app.use('/api/traces', (_req: Request, res: Response) => sendV1Disabled(res, 'traces'));
  } else {
