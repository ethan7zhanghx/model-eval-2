...
export default function EvalsTable({
  ...
  const handleExportCSV = useCallback(() => {
    const headers = ['ID', '创建时间', '类型', '说明', '通过率', '测试数'];
    const csvRows = [
      headers.join(','),
      ...rows.map((row) =>
        [
    ...
    () => [
      ...
      ...(hasRedteamEvals
        ? [
            {
              id: 'type',
              accessorFn: (row) => (row.isRedteam ? '红队' : '评测'),
              header: '类型',
              cell: ({ row }) => {
  ...
  return (
    <>
      ...
      <Dialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>删除 {selectedEvalIds.length} 条评测记录？</DialogTitle>
            <DialogDescription>确定要删除选中的评测记录吗？此操作不可撤销。</DialogDescription>
          </DialogHeader>
          <DialogFooter>
