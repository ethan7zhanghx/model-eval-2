...
export default function EvalOutputPromptDialog({
  ...
  const handleReplay = async () => {
    if (!evaluationId || !provider) {
      setReplayError('缺少评测 ID 或模型来源信息');
      return;
    }

    if (!onReplay) {
      setReplayError('当前版本暂不支持重新运行');
      return;
    }
    ...
    try {
      ...
      if (result.error) {
        ...
        setReplayOutput('（本次未返回输出内容）');
      }
    } catch (error) {
      setReplayError(error instanceof Error ? error.message : '运行过程中发生错误');
    } finally {
    ...
    <Sheet open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      ...
      >
        ...
        <SheetHeader className="flex flex-row items-center justify-between p-4 border-b border-border space-y-0">
          <SheetTitle>详情{provider && `：${provider}`}</SheetTitle>
          <SheetDescription className="sr-only">
            查看 Prompt、输出结果、评测详情与元数据
          </SheetDescription>
          <Button
            variant="ghost"
            size="icon"
        ...
        >
          <TabsList className="w-full justify-start rounded-none border-b border-border bg-transparent px-4 h-auto py-0">
            ...
            >
              {hasOutputContent ? 'Prompt 与输出' : 'Prompt'}
            </TabsTrigger>
            {hasEvaluationData && (
              <TabsTrigger
                value="evaluation"
