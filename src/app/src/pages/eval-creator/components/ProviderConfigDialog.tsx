        ...
        <div className="space-y-4 py-4">
          {isAzureProvider && (
            <Alert variant={isDeploymentIdValid ? 'default' : 'destructive'}>
              <AlertDescription>
                {isDeploymentIdValid
                  ? 'Azure OpenAI 需要填写与 Azure 门户中部署名称一致的 deployment ID。'
                  : 'Azure OpenAI 模型必须填写 deployment ID，它就是你在 Azure 门户中为模型部署设置的名称。'}
              </AlertDescription>
            </Alert>
          )}
                  ...
                  {isAgentIdValid && isAgentAliasIdValid ? (
                    <ul className="mt-2 ml-4 list-disc space-y-1 text-sm">
                      <li>
                        ...
                        {(localConfig.agentId as string) || '未填写'}
                      </li>
