...
export class OpenRouterProvider extends OpenAiChatCompletionProvider {
  ...
  private async executeOpenRouterCall(
    ...
    logger.info('[openrouter] preparing request', {
      providerId: this.id(),
      hasApiKey: Boolean(this.getApiKey()),
      apiKeyEnvar: this.config.apiKeyEnvar,
      envKeys: Object.keys((this.env || {}) as Record<string, unknown>),
    });
    ...
    logger.info('[openrouter] request headers prepared', {
      providerId: this.id(),
      ...
      authorizationHeaderPrefix:
        typeof requestHeaders.Authorization === 'string'
          ? requestHeaders.Authorization.slice(0, 12)
          : undefined,
      configHeaderKeys: Object.keys((config.headers || {}) as Record<string, unknown>),
    });
